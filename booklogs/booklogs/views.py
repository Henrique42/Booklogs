...
def profile(request):
    ...
    # Initialize the Axis for graphs
    # X-Axis is months, Y-Axis is books read
    months = [i+1 for i in range(12)]
    books_read = [0 for _ in range(12)]

    # Set the value foe books read per month on Y-Axis
...
# Export the books read by the current user by a xlsx file
@login_required
def reading_history(request):
    ...
    response['Content-Disposition'] = 'attachment; filename=read_history.xlsx'
